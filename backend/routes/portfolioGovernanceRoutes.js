...
// Portfolio-wide governance/operating endpoints: executive summary, operational
// history, operational change feed, behavioral stability, maturity index, and
// the aggregate engineering-governance score. Split out of portfolioRoutes.js
...
//
// GET /governance internally re-runs the same per-repo architecture forecast
// pipeline (trendTimeline → regressions → couplingAlerts → forecast → anomaly)
// used by portfolioArchitectureRoutes.js's GET /forecast and /watchlists, so
// this file independently imports several `execution/architecture/*` modules
// too — that is expected import duplication across two routers calling the
// same execution module, not a shared helper that needed extracting.
...
// Used only by GET /governance below.
...
// Used only by GET /executive-summary below.
