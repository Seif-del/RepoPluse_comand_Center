 ...
 * On conflict: updates github_username, email, updated_at, and clears deleted_at.
 ...
 * @param {string|number} params.githubId      - GitHub numeric user ID (non-empty string or finite number)
 * @param {string}       params.githubUsername  - GitHub login handle (non-empty string)
 * @param {string|null}  params.email          - Primary email from GitHub; null if hidden
 * @param {string}       params.defaultRole    - Role assigned on first insert (non-empty string)
 * @param {Date}         params.now            - Current timestamp for created_at / updated_at
 ...
 */
async function upsertUser({ db, githubId, githubUsername, email, defaultRole, now } = {}) {
  ...
  const result = await db.query(
    `INSERT INTO users
       (github_id, github_username, email, role, created_at, updated_at, deleted_at)
     ...
     ON CONFLICT (github_id) DO UPDATE SET
       ...
       deleted_at      = NULL
     RETURNING *`,
    [githubId, githubUsername, email, defaultRole, now]
  );
