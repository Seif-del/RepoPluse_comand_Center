...
Last updated: **2026-06-17** (FR-009 activity recency filter — backend + frontend integrated)
...
**Test Status:** 6,676 / 6,708 passing under `npm test` (32 skipped = 25 integration DB + 7 SMTP opt-in; 0 failing) — updated 2026-06-17 (+34 new tests this session: 12 backend activeSince unit + 10 backend activeSince HTTP contract + 12 frontend activeSince filter/load); SMTP integration suite (`tests/integration/notifications.smtp.integration.test.js`, 7 tests) adds 7 opt-in tests (skipped under `npm test`, 7/7 passing with `TEST_INTEGRATION=true` + Mailhog running)  
...
| FR-009 | Search and Filtering | Partially Implemented — Risk-level filtering + Repository name search + Activity recency: **Integrated / Tested** (2026-06-17); label/risk filter tested (29/29); backend route tests 282/282; frontend filter/load tests 65/65; At Risk semantics unchanged: client-side `critical \|\| at-risk` (by design in Option A); **activity recency filter added 2026-06-17**: `?activeSince=` param (7d/30d/90d/stale); stale includes `last_synced_at IS NULL` or older than 30 days; `#repo-recency-select` dropdown in filter bar; `buildReposUrl` supports riskLevel + search + activeSince ordering; 3 spec dimensions still absent: project status, assigned manager, intern contributor |
...
- **Capability:** Search and Filtering (FR-009 — Partially Implemented; risk-level filtering + repository name search + activity recency: Integrated / Tested)
  ...
  - Tests: `tests/unit/backend/routes/repoRoutes.test.js` — **256/256 passing** (updated 2026-06-17: 8 new search filter tests + 12 new activeSince unit tests added; 9 existing param-array assertions updated to 5-element)
  ...
  - Tests: `tests/unit/backend/routes/repoRoutes.http.test.js` — **26/26 passing** (updated 2026-06-17: 10 new activeSince HTTP contract tests added; 4 existing param-array assertions updated to 5-element; was 16/16)
  - Missing: At Risk toggle intentionally uses client-side `critical || at-risk` (by design in Option A); filter by assigned manager, intern contributor, project status absent; SQL filter correctness against real PostgreSQL unverified (no integration test yet — `rs.label = $2`, `ILIKE '%' || $3 || '%'`, and `last_synced_at` timestamp clauses unproven against live DB)
...
| Backend (routes, middleware) | ~10 | ~682 | All passing |
...
| **Total (`npm test`)** | **97 passing + 3 skipped = 100** | **6,708** | **6,676 passing, 32 skipped (25 DB + 7 SMTP, all opt-in), 0 failing** |
...
| FR-009 filter incomplete | Medium | Label/risk filter tested (29/29 passing, 2026-06-12). Backend route tests (282/282 passing, updated 2026-06-17). Frontend filter/load tests (65/65 passing, updated 2026-06-17). **Risk-level filtering + Repository name search + Activity recency are now Integrated / Tested.** ~~repository name search absent~~ — **resolved 2026-06-17**: `?search=` param with ILIKE on `github_full_name`; 200-char limit + HTTP 400 on invalid; `#repo-search-input` in filter bar. ~~activity recency filter absent~~ — **resolved 2026-06-17**: `?activeSince=` param (7d/30d/90d/stale); stale includes `last_synced_at IS NULL`; `#repo-recency-select` dropdown; `buildReposUrl` supports riskLevel + search + activeSince ordering; 282/282 backend + 65/65 frontend tests passing. At Risk toggle intentionally uses client-side `critical \|\| at-risk` (by design in Option A). SQL filter clauses unverified against real PostgreSQL (no integration test). 3 spec dimensions still absent: project status, assigned manager, intern contributor. |
