...
**Test Status:** 6,786 / 6,856 passing serially under `npm test --runInBand` (70 skipped = 63 integration DB + 7 SMTP opt-in; 0 failing) — updated 2026-06-19 (+11 distribution-based response guard tests; portfolioRoutes.test.js forecast block now 49/49 passing; parallel run has pre-existing Jest worker race on 5 unrelated suites — all pass in isolation and serially); DB integration suite total: 63 opt-in tests across 4 files; SMTP integration suite 7 opt-in tests (skipped under `npm test`, 7/7 passing with `TEST_INTEGRATION=true` + Mailhog running)  
