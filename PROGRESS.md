...
**Test Status:** 6,723 / 6,793 passing under `npm test` (70 skipped = 63 integration DB + 7 SMTP opt-in; 0 failing) — updated 2026-06-19 (sync persistence bug fix: `0014_project_status.test.js` → 8 tests (+2: exact default value + quote-guard); `fetchUserRepos.test.js` → 25; `repoRoutes.test.js` → 271; prior sync fix: 11 new `fetchUserRepos` tests + 3 new `repoRoutes` tests); DB integration suite total: 63 opt-in tests across 4 files; SMTP integration suite 7 opt-in tests (skipped under `npm test`, 7/7 passing with `TEST_INTEGRATION=true` + Mailhog running)  
